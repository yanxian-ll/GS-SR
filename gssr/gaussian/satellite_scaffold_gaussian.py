...
@dataclass
class SatelliteScaffoldGaussianConfig(VanillaGaussianConfig):
    _target: type = field(default_factory=lambda: SatelliteScaffoldGaussian)
...
class SatelliteScaffoldGaussian(VanillaGaussian):
    config: SatelliteScaffoldGaussianConfig
    ...
    def __init__(self, config: VanillaGaussianConfig, device: str = 'cuda', world_size: int = 1, local_rank: int = 0) -> None:
        ...
        self.mapping_size = [10, 4]
        self.mapping_xyz = Mapping(self.mapping_size[0], 3)
        self.mapping_xyz_dim = 2 * self.mapping_size[0] * 3

        self.mapping_view = Mapping(self.mapping_size[1], 3)
        self.mapping_view_dim = 2* self.mapping_size[1] * 3

        self.mlp_feature_bank = nn.Sequential(
                nn.Linear(self.feat_dim + self.mapping_xyz_dim, self.feat_dim),
                nn.ReLU(True),
        ...
        self.mlp_opacity = nn.Sequential(
            ...
            # nn.Tanh()
            nn.Linear(self.feat_dim+self.mapping_view_dim, self.feat_dim//2),
            nn.ReLU(True),
            nn.Linear(self.feat_dim//2, self.n_offsets),
            nn.Tanh()
        ).to(device)
        ...
        self.mlp_cov = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Linear(self.feat_dim//2, 7*self.n_offsets),
        ).to(device)
        ...
        self.mlp_sky_color = nn.Sequential(
            torch.nn.Linear(self.mapping_view_dim, self.feat_dim // 2),
            torch.nn.ReLU(),
            torch.nn.Linear(self.feat_dim // 2, 3),
            torch.nn.Sigmoid(),
        ).to(device)
        ...
        self.mlp_transient_encoding = nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(self.feat_dim//2, self.feat_dim//2),
            torch.nn.ReLU(),
            # torch.nn.Linear(self.feat_dim//2, self.feat_dim//2),
            # torch.nn.ReLU()
        ).to(device)

        self.mlp_transient = nn.Sequential(
            torch.nn.Linear(self.feat_dim//2, self.n_offsets*3),
            torch.nn.Sigmoid(),
        ).to(device)

        self.mlp_uncertainty = nn.Sequential(
            torch.nn.Linear(self.feat_dim//2, self.n_offsets),
            torch.nn.Softplus(),
        ).to(device)
    ...
    def set_appearance(self, num_cameras):
        ...
        affine[:, 1:2] *= 1e-10
        self.color_affine = nn.Parameter(affine.requires_grad_(True))
            
    @property
    def get_mapping_xyz(self):
        return self.mapping_xyz
    
    @property
    def get_mapping_view(self):
        return self.mapping_view
    ...
    @property
    def get_color_affine(self):
        return self.color_affine
    ...
    @property
    def get_sky_color_mlp(self):
        return self.mlp_sky_color
    
    @property
    def get_transient_encoding_mlp(self):
        return self.mlp_transient_encoding

    @property
    def get_transient_mlp(self):
        return self.mlp_transient
    ...
    def setup_optimizers(self):
        ...
        l = [
            ...
            {'params': self.mlp_sky_color.parameters(), 'lr': self.config.mlp_color_lr_init*0.001, "name": "mlp_sky_color"},

            {'params': self.mlp_transient_encoding.parameters(), 'lr': self.config.mlp_color_lr_init*0.001, "name": "mlp_transient_encoding"},
            {'params': self.mlp_transient.parameters(), 'lr': self.config.mlp_color_lr_init*0.001, "name": "mlp_transient"},
            {'params': self.mlp_uncertainty.parameters(), 'lr': self.config.mlp_color_lr_init*0.001, "name": "mlp_uncertainty"},

            {'params': self.color_affine, 'lr': self.config.mlp_color_lr_init*0.001, "name": "embedding_affine"},
        ...
        self.mlp_sky_scheduler_args = get_expon_lr_func(lr_init=self.config.mlp_color_lr_init*0.001,
                                                    lr_final=self.config.mlp_color_lr_final*0.01,
                                                    lr_delay_mult=self.config.mlp_color_lr_delay_mult,
                                                    max_steps=self.config.mlp_color_lr_max_steps)
        
        self.mlp_transient_encoding_scheduler_args = get_expon_lr_func(lr_init=self.config.mlp_color_lr_init*0.001,
                                                    lr_final=self.config.mlp_color_lr_final*0.01,
                                                    lr_delay_mult=self.config.mlp_color_lr_delay_mult,
                                                    max_steps=self.config.mlp_color_lr_max_steps)
        
        self.mlp_transient_scheduler_args = get_expon_lr_func(lr_init=self.config.mlp_color_lr_init*0.001,
                                                    lr_final=self.config.mlp_color_lr_final*0.01,
                                                    lr_delay_mult=self.config.mlp_color_lr_delay_mult,
                                                    max_steps=self.config.mlp_color_lr_max_steps)
        self.mlp_uncertainty_scheduler_args = get_expon_lr_func(lr_init=self.config.mlp_color_lr_init*0.001,
                                                    lr_final=self.config.mlp_color_lr_final*0.01,
                                                    lr_delay_mult=self.config.mlp_color_lr_delay_mult,
                                                    max_steps=self.config.mlp_color_lr_max_steps)
        ...
        self.affine_scheduler_args = get_expon_lr_func(lr_init=self.config.mlp_color_lr_init*0.001,
                                                    lr_final=self.config.mlp_color_lr_final*0.01,
                                                    lr_delay_mult=self.config.mlp_color_lr_delay_mult,
                                                    max_steps=self.config.mlp_color_lr_max_steps)
    def update_learning_rate(self, step):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "mlp_color":
                ...
                param_group['lr'] = lr
            if param_group["name"] == "mlp_featurebank":
            ...
            if param_group['lr'] == "mlp_sky_color":
                lr = self.mlp_sky_scheduler_args(step)
                param_group['lr'] = lr

            if param_group['lr'] == "mlp_transient_encoding":
                lr = self.mlp_transient_encoding_scheduler_args(step)
                param_group['lr'] = lr
            if param_group['lr'] == "mlp_transient":
                lr = self.mlp_transient_scheduler_args(step)
                param_group['lr'] = lr
            if param_group['lr'] == "mlp_uncertainty":
                lr = self.mlp_uncertainty_scheduler_args(step)
                param_group['lr'] = lr
    ...
    def save_mlp_checkpoints(self, path):
        if self.config.save_ckpt_mode == 'split':
            ...
            self.mlp_feature_bank.eval()
            feature_bank_mlp = torch.jit.trace(self.mlp_feature_bank, (torch.rand(1, self.feat_dim+self.mapping_xyz_dim).cuda()))
            feature_bank_mlp.save(os.path.join(path, 'feature_bank_mlp.pt'))
            self.mlp_feature_bank.train()

            self.mlp_opacity.eval()
            opacity_mlp = torch.jit.trace(self.mlp_opacity, (torch.rand(1, self.feat_dim+self.mapping_view_dim).cuda()))
            opacity_mlp.save(os.path.join(path, 'opacity_mlp.pt'))
            self.mlp_opacity.train()

            self.mlp_cov.eval()
            cov_mlp = torch.jit.trace(self.mlp_cov, (torch.rand(1, self.feat_dim).cuda()))
            ...
            color_mlp.save(os.path.join(path, 'sky_mlp.pt'))
            self.mlp_sky_color.train()

            self.mlp_transient_encoding.eval()
            transient_encoding_mlp = torch.jit.trace(self.mlp_transient_encoding, (torch.rand(1, self.feat_dim+self.appearance_dim).cuda()))
            transient_encoding_mlp.save(os.path.join(path, 'transient_encoding_mlp.pt'))
            self.mlp_transient_encoding.train()

            self.mlp_transient.eval()
            transient_mlp = torch.jit.trace(self.mlp_transient, (torch.rand(1, self.feat_dim//2).cuda()))
            transient_mlp.save(os.path.join(path, 'transient_mlp.pt'))
            self.mlp_transient.train()

            self.mlp_uncertainty.eval()
            uncertainty_mlp = torch.jit.trace(self.mlp_uncertainty, (torch.rand(1, self.feat_dim//2).cuda()))
            uncertainty_mlp.save(os.path.join(path, 'uncertainty_mlp.pt'))
            self.mlp_uncertainty.train()
            ...
            if self.appearance_dim > 0:
                torch.save({
                    ...
                    'sky_mlp': self.mlp_sky_color.state_dict(),
                    'feature_bank_mlp': self.mlp_feature_bank.state_dict(),

                    'transient_encoding_mlp': self.mlp_transient_encoding.state_dict(),
                    'transient_mlp': self.mlp_transient.state_dict(),
                    'uncertainty_mlp': self.mlp_uncertainty.state_dict(),
                    'appearance': self.embedding_appearance.state_dict(),
                    'affine': self.color_affine,
    ...
    def load_mlp_checkpoints(self, path):#split or unite
        if self.config.save_ckpt_mode == 'split':
            ...
            self.mlp_sky_color = torch.jit.load(os.path.join(path, 'sky_mlp.pt')).cuda()
            
            self.mlp_transient_encoding = torch.jit.load(os.path.join(path, 'transient_encoding_mlp.pt')).cuda()
            self.mlp_transient = torch.jit.load(os.path.join(path, 'transient_mlp.pt')).cuda()
            self.mlp_uncertainty = torch.jit.load(os.path.join(path, 'uncertainty_mlp.pt')).cuda()
            
            self.color_affine = torch.load(os.path.join(path, 'color_affine.pt')).cuda()
            ...
            self.mlp_sky_color.load_state_dict(checkpoint['sky_mlp'])

            self.mlp_transient_encoding.load_state_dict(checkpoint['transient_encoding_mlp'])
            self.mlp_transient.load_state_dict(checkpoint['transient_mlp'])
            self.mlp_uncertainty.load_state_dict(checkpoint['uncertainty_mlp'])
            self.color_affine = torch.load(checkpoint['affine'])
