...
from gssr.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation, strip_symmetric, build_scaling_rotation
from gssr.utils.system_utils import mkdir_p
from gssr.utils.sh_utils import RGB2SH
...
from gssr.gaussian.base_gaussian import GaussianModel, GaussianModelConfig

from simple_knn._C import distCUDA2


@dataclass
class VanillaGaussianConfig(GaussianModelConfig):
    _target: type = field(default_factory=lambda: VanillaGaussian)
    ...
    oneupSHdegree_interval: int = 1000
...
class VanillaGaussian(GaussianModel):
    config: VanillaGaussianConfig
    ...
    def setup_optimizers(self):
        ...
        l = [
            {'params': [self._xyz], 'lr': self.config.position_lr_init * self.spatial_lr_scale, "name": "xyz"},
            {'params': [self._features_dc], 'lr': self.config.feature_lr, "name": "f_dc"},
            {'params': [self._features_rest], 'lr': self.config.feature_lr / 20.0, "name": "f_rest"},
        ...
        self.xyz_scheduler_args = get_expon_lr_func(lr_init=self.config.position_lr_init*self.spatial_lr_scale,
    ...
    def restore(self, model_args):
        ...
        # self.setup_optimizers(training_args)
    ...
    def get_training_callbacks(self) -> List[TrainingCallback]:
        ...
        callbacks.append(
            TrainingCallback(
                label="oneupSHdegree",
                where_to_run=[TrainingCallbackLocation.BEFORE_TRAIN_ITERATION],
                update_every_num_iters=self.config.oneupSHdegree_interval,
                func=self.oneupSHdegree,
            )
        )
        return callbacks
    ...
    def densify(self, step, **kwargs):
        ...
        if step < self.config.densify_until_iter:
            ...
            if step > self.config.densify_from_iter and step % self.config.densification_interval == 0:
                ...
                self.densify_and_prune(self.config.densify_grad_threshold, self.config.opacity_cull_threshold, self.spatial_lr_scale, size_threshold)
