	...
	const bool debug)
{
  ...
  torch::Tensor out_weight = torch::full({0, H, W}, 0.0, float_opts);
  out_weight = torch::full({1, H, W}, 0.0, float_opts).contiguous();
  float* out_weightptr = nullptr;
  out_weightptr = out_weight.data<float>();
  ...
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth, out_weight);
}
