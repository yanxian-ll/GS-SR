	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (T > 0.5) {
				median_depth = depth;
				// median_weight = w;
				surf_idx = collected_id[j];
				for (int ch=0; ch<3; ch++) median_normal[ch] = normal[ch];
	...
	if (inside)
	{
		...
		out_others[pix_id + MIDIDX_OFFSET * H * W] = surf_idx;
		for (int ch=0; ch<3; ch++) out_others[pix_id + (MIDNORMAL_OFFSET+ch) * H * W] = median_normal[ch];
#endif
	}
}
